...
- **Every value rendered came off the network — and `actor` comes off the URL.** Escape both. `lib/embed.js` exports `esc`; use it for every interpolation, in `wall.js` as much as in `render.js`. An unescaped `actor` on a page built to be a public link is reflected XSS.
